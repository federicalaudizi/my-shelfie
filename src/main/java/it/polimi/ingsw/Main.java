        ...
        if (args.length == 0){
            ...
            if (args[0].equals("--client") || args[0].equals("-c")) {
                ...
                if(args.length == 1) new ClientSocket(false).start();
                    ...
                    else System.out.println("invalid option " + args[1] + "\n" + "To be provided: '--socket/-s' or '--rmi/-r'");
                } else if(args.length == 3) {
                    // Socket and GUI
                    if ((args[1].equals("--socket") || args[1].equals("-s")) && (args[2].equals("--gui") || args[2].equals("-g"))) new ClientSocket(false).start();
                    // Socket and TUI
                    else if ((args[1].equals("--socket") || args[1].equals("-s")) && (args[2].equals("--tui") || args[2].equals("-t"))) new ClientSocket(true).start();
                    // RMI and GUI
                    else if ((args[1].equals("--rmi") || args[1].equals("-r")) && (args[2].equals("--gui") || args[2].equals("-g"))) new ClientRMI(false).start();
                    // RMI and TUI
                    else if ((args[1].equals("--rmi") || args[1].equals("-r")) && (args[2].equals("--tui") || args[2].equals("-t"))) new ClientRMI(true).start();
                    else {
